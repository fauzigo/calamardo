 ...
 * squid.class.php
 * 
 * Class in charge to get the squid config file and get the data included.
 * 
 * 
 * @author pbujanda@apogeesystem.net
 * @since 1.0
 * 
 * 
 */
...
class squidConf {
	...
	function getRulesTypes($typeName=NULL,$listRares=FALSE){
		$ini=parse_ini_file("rules.ini", true);
		if(!$typeName){
			foreach($ini as $name => $content){
	...
	function getRulesClass($className=NULL){
		$ini=parse_ini_file("rules.ini", true);
		if(!$className){
			foreach($ini as $name => $content){
				switch ($content['class']){
